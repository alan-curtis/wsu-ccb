...
<li <?php post_class($class.' '.ex_wptl_lightbox($full_content,$ID,'class'));?> <?php echo 'data-id="filter-'.$ID.'_'.get_the_ID().'"';?> <?php echo ex_wptl_lightbox($full_content,$ID,'data'); ?>>
	<div class="<?php echo esc_attr($animations);?> tl-ani">
        ...
        <div class="wpex-timeline-label">
            <?php if( ($show_media!='1' && $hide_img!=1 && has_post_thumbnail(get_the_ID())) || ($show_media=='1' && $hide_img!=1 && has_post_thumbnail(get_the_ID()) && wptl_audio_video_iframe()=='<div class="wptl-embed"></div>') && ex_wptl_image_gallery()==''){ ?>
                <a href="<?php echo extl_dataes($link_lb!='' ? $link_lb : $custom_link);?>" title="<?php the_title_attribute();?>" class="il-img">
