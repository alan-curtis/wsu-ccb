...
<li <?php post_class($class.' exif-li '.ex_wptl_lightbox($full_content,$ID,'class'));?> <?php echo 'data-id="filter-'.$ID.'_'.get_the_ID().'"';?> <?php echo ex_wptl_lightbox($full_content,$ID,'data'); ?>>
    <div class="<?php echo esc_attr($animations);?> tl-ani">
        ...
        <div class="tlif-contai">
            <div class="tlif-img" <?php echo extl_dataes($bg_style);?>>
            ...
            <div class="tlif-content">
                <div class="tlif-details">
                    ...
                    <div class="tlif-info">
                        <?php if($wpex_felabel!=''){
                            echo '<div class="extlif-felname"><span>'.extl_dataes($wpex_felabel,true).'</span></div>';
                        }?>
                        <h2 class="tlif-title">
    ...
    <?php if($we_eventcolor!=''|| $wpex_icon_img!=''){?>
        <style type="text/css">
            ...
            if($we_eventcolor!=''){?>
                ...
                #timeline-<?php echo esc_attr($ID);?> li.post-<?php the_ID();?> .tlif-content .tlif-media,
                #timeline-<?php echo esc_attr($ID);?> li.post-<?php the_ID();?> .extlif-felname span,
                #timeline-<?php echo esc_attr($ID);?> li.post-<?php the_ID();?>  .tlif-img{border-color:<?php echo esc_attr($we_eventcolor);?>;}
                #timeline-<?php echo esc_attr($ID);?> li.post-<?php the_ID();?> .tlif-img,
                #timeline-<?php echo esc_attr($ID);?> li.post-<?php the_ID();?> span.tlif-icon{background-color:<?php echo esc_attr($we_eventcolor);?>;}
                <?php 
            }?>
        </style> 
    <?php } ?>
