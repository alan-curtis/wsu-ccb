...
function ajax_wpex_loadmore_timeline(){
	...
	$output =  array('html_content'=>$html,'date'=> $ft_date,'count'=>$it);
...
function ajax_wpex_filter_taxonomy(){
	...
	$output =  array('html_content'=>$html,'date'=> $ft_date,'more'=> $it > $posts_per_page ? 1: 0, 'data_query'=> $args,'count'=>$it_co);
	echo str_replace('\/', '/', json_encode($output));
	die;
}
// filter year
...
if(!function_exists('wpex_safe_strtotime')){
	function wpex_safe_strtotime($string,$fm)
	{
		...
		if($fm != ''){
			...
			$exwptl_datefm = exwptl_get_option('exwptl_datefm','exwptl_advanced_options');
			$date_fm = $exwptl_datefm!='' ? $exwptl_datefm : get_option('date_format');
		}
...
// fix icon image in version 3.4 with cmb2
if(!function_exists('wptl_change_icon_meta')){
	function wptl_change_icon_meta($metadata, $object_id, $meta_key, $single) {
		if($meta_key=='wpex_icon_img' ) {
			...
			if(!is_numeric($metadata)){
				$metadata = get_post_meta( $object_id, 'wpex_icon_img_id', true );
			}
		}
		return $metadata;
	}
...
if(!is_admin() || (defined('DOING_AJAX') && DOING_AJAX) ){
	add_filter('get_post_metadata', 'wptl_change_icon_meta', 10, 4);
}

if(!function_exists('extl_custom_color')){
	function extl_custom_color($metadata, $object_id, $meta_key, $single){
		if($meta_key=='we_eventcolor' ) {
			//use $wpdb to get the value
			global $wpdb;
			$metadata = $wpdb->get_var( $wpdb->prepare( 
				"
			...
			if($metadata==''){
				...
				$terms = wp_get_post_terms($object_id, 'wpex_category', $args);
				if(!empty($terms) && !is_wp_error( $terms )){
					foreach ( $terms as $term ) {
						$we_eventcolor = get_term_meta($term->term_id,'extl_cat_color',true);
						if($we_eventcolor!=''){
							$metadata = $we_eventcolor;
							break;
						}
					}
				}
			}
			$metadata = apply_filters( 'extl_customcolor', $metadata,$object_id );
		}
		return $metadata;
	}
	if(!is_admin() || (defined('DOING_AJAX') && DOING_AJAX) ){
		add_filter('get_post_metadata', 'extl_custom_color', 10, 4);
	}
}

add_filter('wptimeline_link','extl_disable_link_sgtimeline');
add_filter('wptl_link_in_fullcontent','extl_disable_link_sgtimeline');
function extl_disable_link_sgtimeline($link){
	$dis_link = get_post_meta( get_the_ID(), 'wpex_dis_link', true );
	if($dis_link =='yes'){
		$link ='#';
	}
	return $link;
}
add_filter('extl_disable_lightbox','extl_disable_lb_sgtimeline');
function extl_disable_lb_sgtimeline($disable){
	$dis_link = get_post_meta( get_the_ID(), 'wpex_dis_link', true );
	if($dis_link =='yes'){
		$disable ='1';
	}
...
function extl_dataes($content,$escape=false){
