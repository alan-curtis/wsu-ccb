...
if(!class_exists('WPTL_Hozizontal_Multi_Elementor_Widget')){
	class WPTL_Hozizontal_Multi_Elementor_Widget extends Widget_Base {
		...
		public function get_title() {
			return esc_html__( 'Timeline horizontal Multi items', 'wp-timeline');
		}
		...
		protected function _register_controls() {
			$this->start_controls_section(
				...
				[
					'label' 		=> esc_html__( 'Timeline horizontal Multi items', 'wp-timeline')
				]
			);
			$this->add_control(
				'style',
				[
					...
					'options'		=> array(
						...
						'7' => esc_html__('Style 7', 'wp-timeline'),
					),
				]
			);
			$args = array(
		...
		protected function render() {
			...
			echo do_shortcode('[timeline_horizontal_multi style="'.$style.'" posttype="'.$posttype.'" cat="'.$cat.'" tag="'.$tag.'" taxonomy="'.$taxonomy.'" ids="'.$ids.'" count="'.$count.'" order="'.$order.'" orderby="'.$orderby.'" meta_key="'.$meta_key.'" autoplay="'.$autoplay.'" show_media="'.$show_media.'" show_label="'.$show_label.'" full_content="'.$full_content.'" hide_thumb="'.$hide_thumb.'" arrow_position="'.$arrow_position.'" autoplayspeed="'.$autoplayspeed.'" start_on="'.$start_on.'" slidesshow="'.$slidesshow.'" loading_effect="'.$loading_effect.'" enable_back="'.$enable_back.'" class="'.$class.'"]').'<script>if (typeof (Wptl_El_Sp) !== "undefined" && Wptl_El_Sp!==null) { Wptl_El_Sp.timelines_hoz_multi(); }</script>';
		}
